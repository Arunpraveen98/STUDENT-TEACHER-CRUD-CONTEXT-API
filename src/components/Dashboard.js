...
function StudentDashboard() {
  ...
  //----------------------------------------------------------
  //? Add Teacher data button...
  const addingTeacherData = () => {
    if (!localStorage.getItem("user_name")) {
      history.replace("/loginPage");
    } else {
      history.push("./addTeacherData");
    }
  };
  //----------------------------------------------------------
  //? Teachers lists button...
  const viewingTeachersList = () => {
    if (!localStorage.getItem("user_name")) {
      history.replace("/loginPage");
    } else {
    ...
    <div className="Dashboard">
      ...
      <div className="icons">
        ...
        <button
          type="button"
          className="dash-btn"
          onClick={() => addingTeacherData()}
        >
          <GroupAddIcon className="dash-icon" />
          <br></br>
          <span className="dash-txt">ADD TEACHER</span>
        </button>

        <button
          type="button"
          className="dash-btn"
          onClick={() => viewingTeachersList()}
        >
          <RecentActorsIcon className="dash-icon" />
          <br></br>
          <span className="dash-txt">TEACHER LISTS</span>
        </button>
      </div>
    </div>
  );
